...
export default function Track() {
    ...
    if (loader) {
        return (<div className="flex justify-center items-center min-h-screen bg-gradient-to-br from-purple-50 to-pink-100">
    ...
    const trackMedicine = async (medicineId) => {
        try {
    ...
    const stageIcons = {
        rms: (<svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/>
      </svg>),
        manufacture: (<svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z"/>
      </svg>),
        distribute: (<svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"/>
      </svg>),
        retail: (<svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/>
      </svg>),
        sold: (<svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
      </svg>),
    };
    const TrackComponent = ({ title, stages, }) => {
        const medicineId = parseInt(id);
        const batteryData = {
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 mb-6">
          ...
          <div className="relative">
            ...
            <div className="space-y-8">
              {stages.map((stage, index) => (<div key={index} className="relative flex items-start">
                  {/* Timeline dot */}
                  <div className="relative z-10 flex-shrink-0 w-16 h-16 bg-gradient-to-br from-purple-500 to-pink-500 rounded-full flex items-center justify-center shadow-lg transform hover:scale-110 transition-transform">
                    {stage.icon || (<div className="w-8 h-8 bg-white rounded-full flex items-center justify-center">
                  ...
                  <div className="ml-6 flex-1 bg-gray-50 rounded-xl p-6 shadow-md hover:shadow-lg transition-shadow">
                    <h5 className="text-lg font-bold text-gray-800 mb-3 flex items-center">
                      {stage.label}
                      {stage.data && (<span className="ml-2 px-2 py-1 bg-green-100 text-green-700 text-xs font-semibold rounded-full">
                          ✓ Completed
                        </span>)}
                    </h5>
                    {stage.data ? (<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      </div>) : (<div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded">
                        <p className="text-yellow-800 font-medium">⏳ Not yet processed</p>
                      </div>)}
                  </div>
                  
                  {/* Arrow connector */}
                  {stage.showArrow && index < stages.length - 1 && (<div className="hidden md:block absolute left-8 top-16 w-0.5 h-8 bg-gradient-to-b from-purple-400 to-pink-400"></div>)}
                </div>))}
            </div>
          </div>
        </div>
        ...
        {/* QR Code Section */}
        {trackTillOrdered && (<div className="bg-white rounded-2xl shadow-xl p-8 mb-6">
            ...
            <div className="flex justify-center">
              <div className="bg-gradient-to-br from-purple-50 to-pink-50 p-6 rounded-xl border-2 border-purple-200">
                <QRCodeCanvas value={JSON.stringify(batteryData)} size={250} level="H" includeMargin={true}/>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <button onClick={resetTracking} className="flex-1 px-6 py-3 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-xl font-semibold hover:from-green-600 hover:to-emerald-700 transition-all shadow-lg hover:shadow-xl transform hover:scale-105 flex items-center justify-center">
            <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
            </svg>
            Track Another Item
          </button>
          <button onClick={() => router.push('/')} className="flex-1 px-6 py-3 bg-gradient-to-r from-red-500 to-pink-600 text-white rounded-xl font-semibold hover:from-red-600 hover:to-pink-700 transition-all shadow-lg hover:shadow-xl transform hover:scale-105 flex items-center justify-center">
            <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    if (trackTillSold) {
        const medicineId = parseInt(id);
        return (<div className="min-h-screen bg-gradient-to-br from-purple-50 to-pink-100 p-5">
        <TrackComponent title="Sold" stages={[
                {
    ...
    if (trackTillRetail) {
        const medicineId = parseInt(id);
        return (<div className="min-h-screen bg-gradient-to-br from-purple-50 to-pink-100 p-5">
        <TrackComponent title="Retail" stages={[
                {
                    label: 'Raw Materials Supplied by',
                    data: rms[parseInt(med[medicineId]?.RMSid)],
                    showArrow: true,
    ...
    if (trackTillDistribution) {
        const medicineId = parseInt(id);
        return (<div className="min-h-screen bg-gradient-to-br from-purple-50 to-pink-100 p-5">
        <TrackComponent title="Distribution" stages={[
                {
                    label: 'Raw Materials Supplied by',
                    data: rms[parseInt(med[medicineId]?.RMSid)],
                    showArrow: true,
    ...
    if (trackTillManufacture) {
        const medicineId = parseInt(id);
        return (<div className="min-h-screen bg-gradient-to-br from-purple-50 to-pink-100 p-5">
        <TrackComponent title="Manufacture" stages={[
                {
                    label: 'Raw Materials Supplied by',
                    data: rms[parseInt(med[medicineId]?.RMSid)],
                    showArrow: true,
    ...
    if (trackTillRMS) {
        const medicineId = parseInt(id);
        return (<div className="min-h-screen bg-gradient-to-br from-purple-50 to-pink-100 p-5">
        <TrackComponent title="RMS" stages={[
                {
                    label: 'Raw Materials Supplied by',
                    data: rms[parseInt(med[medicineId]?.RMSid)],
                    showArrow: false,
    ...
    if (trackTillOrdered) {
        const medicineId = parseInt(id);
        return (<div className="min-h-screen bg-gradient-to-br from-purple-50 to-pink-100 p-5">
        <TrackComponent title="Ordered" stages={[
                {
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 mb-6">
          ...
          <form onSubmit={handlerSubmit} className="flex gap-3">
            <div className="flex-1 relative">
              ...
              <input className="w-full pl-12 pr-4 py-4 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent text-lg" type="text" onChange={handlerChangeID} placeholder="Enter Battery ID (e.g., 1, 2, 3...)" value={id} required/>
            </div>
            <button type="submit" className="px-8 py-4 bg-gradient-to-r from-purple-500 to-pink-600 text-white rounded-xl font-semibold hover:from-purple-600 hover:to-pink-700 transition-all shadow-lg hover:shadow-xl transform hover:scale-105 flex items-center">
              <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7"/>
              </svg>
              Track
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6">
          ...
          {Object.keys(med).length === 0 ? (<div className="text-center py-12">
              ...
              <table className="min-w-full">
                ...
                return (<tr key={key} className="hover:bg-purple-50 transition-colors cursor-pointer" onClick={() => trackMedicine(medicineId)}>
