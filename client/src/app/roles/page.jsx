...
export default function AssignRoles() {
    ...
    const loadBlockchainData = async () => {
        try {
            ...
            const rms = await Promise.all(Array(parseInt(rmsCount))
                .fill(null)
                .map((_, i) => contract.methods.RMS(i + 1).call()));
            const man = await Promise.all(Array(parseInt(manCount))
                .fill(null)
                .map((_, i) => contract.methods.MAN(i + 1).call()));
            const dis = await Promise.all(Array(parseInt(disCount))
                .fill(null)
                .map((_, i) => contract.methods.DIS(i + 1).call()));
            const ret = await Promise.all(Array(parseInt(retCount))
                .fill(null)
    ...
    if (loading) {
        return (<div className="flex justify-center items-center min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-5">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Owner Warning */}
        {!isOwner && (<div className="bg-gradient-to-r from-red-50 to-pink-50 rounded-2xl shadow-lg p-6 mb-6 border-l-4 border-red-500">
        ...
        {/* Registration Form */}
        <div className={`bg-gradient-to-br ${roleConfig[newRole.type].bgGradient} rounded-2xl shadow-xl p-8 mb-6 border-l-4 ${roleConfig[newRole.type].borderColor}`}>
          <div className="flex items-center mb-6">
            <div className={`w-12 h-12 bg-gradient-to-r ${roleConfig[newRole.type].gradient} rounded-xl flex items-center justify-center mr-4 shadow-lg text-white`}>
              {roleConfig[newRole.type].icon}
            </div>
            <div>
          ...
          <form onSubmit={handleRoleSubmit} className="space-y-4">
            <div>
              ...
              <div className="relative">
                <select className="w-full px-4 py-3 pl-12 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent text-lg bg-white appearance-none cursor-pointer" name="type" onChange={handleInputChange} value={newRole.type} required>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input className="w-full pl-12 pr-4 py-3 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent text-lg bg-white font-mono text-sm" type="text" name="address" placeholder="0x..." onChange={handleInputChange} value={newRole.address} required/>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input className="w-full pl-12 pr-4 py-3 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent text-lg bg-white" type="text" name="name" placeholder="Enter participant name" onChange={handleInputChange} value={newRole.name} required/>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input className="w-full pl-12 pr-4 py-3 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent text-lg bg-white" type="text" name="place" placeholder="Enter location (e.g., City, Country)" onChange={handleInputChange} value={newRole.place} required/>
              </div>
            </div>

            <button type="submit" disabled={!isOwner} className={`w-full px-6 py-4 rounded-xl font-semibold transition-all shadow-lg hover:shadow-xl transform ${isOwner
            ? `bg-gradient-to-r ${roleConfig[newRole.type].gradient} text-white hover:scale-105 cursor-pointer`
            : 'bg-gray-400 text-gray-200 cursor-not-allowed'} flex items-center justify-center`}>
              {isOwner ? (<>
                  <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7"/>
                  </svg>
                  Register {roleConfig[newRole.type].label}
                </>) : (<>
                  <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="space-y-6">
          ...
          {['rms', 'man', 'dis', 'ret'].map((roleType) => {
            ...
            return (<div key={roleType} className="bg-white rounded-2xl shadow-xl p-6">
                ...
                {totalCount === 0 ? (<div className="text-center py-12 bg-gray-50 rounded-xl">
                    ...
                    <table className="min-w-full">
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {roleList.map((role, index) => (<tr key={index} className="hover:bg-gray-50 transition-colors">
